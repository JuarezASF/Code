...
Point ColorDetection::DetectColoredObject(Mat &RGB_Input,
                            vector<Scalar> rangeToDetect,
                            int colorThreshold,bool &sucesso){
    /*versão simplificada de detecção de múltiplas cores
    se a área coberta pelo range especificado for menor que colorThreshold,
    então sucesso idicará falso*/
    Point center;
    Mat HSV_Input;
    cvtColor(RGB_Input, HSV_Input, CV_BGR2HSV);


        Scalar colorMin = rangeToDetect[0];
        Scalar colorMax = rangeToDetect[1];
